...
**Автономный ЧПУ-плоттер формата A4 на ESP32 для письма ручкой, карандашом, маркером и пером.** Веб-интерфейс хранится на контроллере и работает без облака: браузер формирует траектории, показывает точный предпросмотр, создаёт G-code и передаёт проверенное задание в FluidNC.
...
- операционный поток «Макет → Инструмент → Станок» с обязательной проверкой перед запуском;
- профили линера, гелевой и шариковой ручек, карандаша, пера с тушью и маркера;
- контуры, одинарная/перекрёстная штриховка и комикс-режим;
- русский однолинейный технический и рукописный шрифты;
...
## Как создаётся G-code

ESP32 хранит и раздаёт интерфейс, а геометрию рассчитывает браузер телефона или компьютера. Готовый G-code записывается на SD-карту, после чего FluidNC выполняет его в реальном времени. Интернет и облако не требуются. Такой подход позволяет обрабатывать SVG и изображения без собственного форка FluidNC и без конкуренции с задачами управления двигателями.

Подробно: [прошивка и G-code](docs/FIRMWARE_AND_GCODE.md), [архитектура](docs/ARCHITECTURE.md).

## Интерфейс оператора

Основной сценарий состоит из трёх видимых этапов:

1. **Макет** — текст, SVG или изображение, поля и предпросмотр.
2. **Инструмент** — профиль движения для реального пишущего узла.
3. **Станок** — подключение, homing, тест пера, рамка, запись и запуск.

Кнопка запуска заблокирована, пока не выполнены физические проверки. Для пера с тушью сохраняется направление штрихов; для карандаша доступны перекрёстная штриховка и повторный проход; для маркера плотность автоматически снижена.

Координаты макета задаются относительно листа, а перед формированием G-code преобразуются в машинные координаты. Номинальное положение центрированного A4 в поле 225 × 315 мм — **X 7,5 мм, Y 9 мм**; после изготовления упора эти значения уточняются в разделе диагностики. Холостые переходы выполняются `G1` с подачей выбранного профиля, а не неконтролируемым rapid `G0`.

Дизайн-контракт: [docs/ART_DIRECTION.md](docs/ART_DIRECTION.md). Руководство оператора: [docs/WEB_INTERFACE.md](docs/WEB_INTERFACE.md).
...
Передавать конфигурацию следует только после проверки точной ревизии платы и распиновки. `config.yaml` предназначен для наладки; после проверки направлений, NC-концевиков и устойчивого homing используется `config-production.yaml` с `must_home: true` и `hard_limits: true`:

```bash
python3 scripts/deploy_webui.py 192.168.1.50 --with-config

# После аппаратной приёмки
python3 scripts/deploy_webui.py 192.168.1.50 --with-config --config-profile production
```
